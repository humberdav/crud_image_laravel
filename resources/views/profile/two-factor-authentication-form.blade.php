<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Agrega seguridad adicional a tu cuenta usando la autenticación en 2 factores.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Finaliza la autenticación en 2 factores.') }}
                @else
                    {{ __('Tienes habilitada la autenticación en 2 factores.') }}
                @endif
            @else
